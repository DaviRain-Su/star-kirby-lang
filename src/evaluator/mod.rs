    ...
    if TypeId::of::<Program>() == type_id {
        ...
        let result = match value {
            Statement::ExpressionStatement(exp) => {
                eval(Box::new(exp.clone()))?
            }
            Statement::LetStatement(let_exp) => {
                eval(Box::new(let_exp.clone()))?
            }
            Statement::ReturnStatement(ret_exp) =>  {
                eval(Box::new(ret_exp.clone()))?
            }
        };
        return Ok(result);
    } else if TypeId::of::<ExpressionStatement>() == type_id {
        trace!("type ExpressionStatement id = {:?}", TypeId::of::<ExpressionStatement>());
        let value = node
            .as_any()
        ...
        match value {
            Expression::PrefixExpression(pre_exp) => {
                return Ok(eval(Box::new(pre_exp.clone()))?)
            }
            ...
            Expression::BooleanExpression(boolean) => {
                return Ok(eval(Box::new(boolean.clone()))?)
            }
            Expression::IfExpression(if_exp) => {
                return Ok(eval(Box::new(if_exp.clone()))?)
            }
            Expression::FunctionLiteral(function) => {
                return Ok(eval(Box::new(function.clone()))?)
            }
            Expression::CallExpression(call_exp) => {
                return Ok(eval(Box::new(call_exp.clone()))?)
            }
        }
    } else if TypeId::of::<IntegerLiteral>() == type_id {
        trace!("type IntegerLiteral id = {:?}", TypeId::of::<IntegerLiteral>());
        let value = node
            .as_any()
